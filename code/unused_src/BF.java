 ...
 * 
 * This is a Bloom Filter Object class and the back bone of this class is the boolean[] bitset.
 * The Vector size and the Number of Filter Elements was fixed in the to 100 for this specific
 * project.
 * The original Bloom Filter class off the Internet used a BitSet Object but this object's
 * size was inconsistent with each incoming String so it was changed to a boolean[]. The
 * bitset implementation is comment.
...
public class BF implements Serializable {
    ...
    private static final int expectedNumberOfFilterElements = 30; // expected (maximum) number of elements to be added
    private int numberOfAddedElements; // number of elements actually added to the Bloom filter
    private int k = (int) Math.round(((double)bitSetSize / (double)expectedNumberOfFilterElements) *
            Math.log(2.0));
    /**
    ...
    public BF(String input) {
    	...
    	if(!input.equals(""))
    	{
			try {
				...
				System.err.println("Error in bloom filter creation");
				e.printStackTrace();
			}
    	}
    ...
    public void createId(String input) throws Exception{
		...
		ArrayList<char[]> charList = MyUtilities.convertToBigrams(input);
		for(char[] a: charList){
				add("" + a[0] + a[1]);
		}
	}
...
//    }
    ...
    public void add(String element) throws Exception {
       ...
       for (int x = 0; x < k; x++) {
    	   hash = Encryption.encrypt(valString, x);

...
public class DatabaseConnection 
{
	...
	private String _odbcConnectionName;
	
	// created to reduce replication and enforce common use
	private DatabaseConnection(String database, String schema) {
		this.database = database;
		this.schema = schema;
	}
	
	// Assumes that the connection is through a trusted AD user, as SQL-level
		// users are not permitted
	public DatabaseConnection(String odbcConnectionName, String database, String schema) {
			this(database, schema);
			setOdbcConnectionName(odbcConnectionName);
			try {
				Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
				this.conn = DriverManager.getConnection(String.format("jdbc:odbc:%s", odbcConnectionName), null, null);
	...
	public String getOdbcConnectionName() {
		return _odbcConnectionName;
	}

	private void setOdbcConnectionName(String _odbcConnectionName) {
		this._odbcConnectionName = _odbcConnectionName;
	}
		...
		try
		{
			...
			if(this.URL.contains("postgres")||this.URL.contains("mysql")){
				...
				if(this.user.toLowerCase().equals("integratedsecurity"))
					URL +=";databaseName=curl;integratedSecurity=true;";
				else
					URL +=";databaseName=curl;user="+this.user+";password="+this.passwd;
	...
	public Object getSingleValueSQL(String strSQL){
		...
		String query = strSQL;
		System.out.print("getSingleValueSQL - Debug: " + query);
		ResultSet rs = null;
		try {			
			if (stmt.execute(query))
			{
	...
	public int getRowCount(String table)
	{
		...
		System.out.println("Debug (getRowCount) : "+"SELECT COUNT(*) AS [COUNT] FROM " + table + " ;");
	...
	public String[] getColumnNames (String table)
	{
		return getColumnNames(
				getTableQuery("SELECT * FROM "+ table + " WHERE 1=0"));
	}
	...
	public String[] getNextResult(ResultSet sqlResults)
	{
		...
		try
		{
			...
			if (sqlResults.next())
			{
				...
				for (int i = 0; i < n; i++)
				{
					System.out.println("Debug (getNextResult) : "+colNames[i]);
					// Add values to the array if they exist
					String colValue = sqlResults.getString(colNames[i]);
					
					if (colValue!=null)
					{
						s[i] = colValue.toUpperCase();
					}
					else
					{

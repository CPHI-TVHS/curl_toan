...
public class PostgreSQLSaver extends AbstractResultsSaver{
	...
	public void saveRow(DataRow row) throws RJException, IOException {	
		...
		System.out.println("DEBUG - SQL Saver - Check if "+targetSchema+"."+targetTable+" exists:" + saverConnection.checkPostgreTableExists(targetTable, targetSchema));
		...
		if(!saverConnection.checkPostgreTableExists(targetTable, targetSchema)){
			...
			System.out.println("DEBUG - Create table statement: "+strCmd);
	...
	public void close() throws IOException {
		...
		int intTP = 1;//resultReporter.calculateTP(targetFullTable, leftLink, rightLink, saverConnection);
		int intFP = 1;//resultReporter.calculateFP(targetFullTable, leftLink, rightLink, saverConnection);
		int intTN = 1;//resultReporter.calculateTN(targetFullTable, leftFullTable, rightFullTable, leftLink, rightLink, saverConnection, targetSchema);
		int intFN = 1;//resultReporter.calculateFN(targetFullTable, leftFullTable, rightFullTable, leftLink, rightLink, saverConnection, targetSchema);
